...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    String path = join(await getDatabasesPath(), 'todo_app_v2.db'); // v2 yaparak db adını değiştirdim, silip yüklemeye gerek kalmasın
    return await openDatabase(
      path,
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      ...
      try {
        await db.execute('ALTER TABLE todos ADD COLUMN isSynced INTEGER NOT NULL DEFAULT 0');
      } catch (e) {
  ...
  Future<int> insertTodo(TodoModel todo) async {
    ...
    return await db.insert(
      'todos', 
      todo.toMap(),
      conflictAlgorithm: ConflictAlgorithm.replace, // ID çakışırsa üzerine yaz (Sync için önemli)
    );
  }
